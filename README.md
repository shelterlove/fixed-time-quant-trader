...
当前代码发布版本为 `v1.3.6`；冻结研究策略版本为 `1.1.0`，对应冻结研究引擎 `1.1.1`。二者不是同一个版本号：前者描述运行代码发布，后者描述不再改动的研究基线。
...
- 仅处理 USDT 报价永续合约，所有内部时间使用 UTC。实时候选集合为正式网可读取行情、且 Binance 测试网实际支持下单的合约交集。
