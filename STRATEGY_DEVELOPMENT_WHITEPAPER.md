...
状态：冻结开发规格 v1.0  
日期：2026-08-31  
用途：作为冻结研究基线从零实现、审查和验收的唯一开发说明。本文不是新的参数研究，也不授权生产网交易。

范围说明（2026-09-02）：仓库现已包含独立的 Binance 测试网执行层，但它不修改本文定义的冻结研究基线。测试网即时市价入场和 24h 多头延长规则分别见 [`README.md`](README.md)、[`LIVE_EXTENSION.md`](LIVE_EXTENSION.md) 与 [`OPERATIONS.md`](OPERATIONS.md)。涉及实时运行时，以代码、`testnet.toml` 和这些运行文档为准。
...
本文定义的冻结基线首先是一套研究和回测系统，不包括交易所密钥、自动下单、仓位同步、故障恢复和实盘风控。当前仓库的测试网执行层是在基线验收后独立实现的运行组件，不构成本文的参数变更，也不支持生产网签名交易。
...
结果基线与策略规则分开维护，以免历史修复后出现两套数字。唯一的当前验收
基线见 [`BASELINE.md`](BASELINE.md)：它记录冻结提交、运行环境、三个授权窗口的
汇总和回归状态。该文件只用于从零实现的结果验收，不构成收益承诺。
