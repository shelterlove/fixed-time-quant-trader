...
3. `E + 4h` (`EXTENSION_CAP`).

No other long, and no short, changes. Extended exits are not used as P90 training observations.
...
The run reads `results/local/research/` and writes only ignored files under `research/exit_protection_extension/results/recent_protection_extension_4h/`:
...
Consider production work only if the variant improves the final chronological 20% after costs, does not materially worsen realized drawdown, and the improvement is not concentrated in one trade. Capacity cost is reported as the number of signals selected in the baseline but no longer selected by the variant.
